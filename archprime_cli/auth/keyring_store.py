...
Service identifier: 'archprime-cli'
...
~/.archprime/credentials.json alongside the free-mode payload, with
mode='premium'. Tokens never appear in plain JSON.

Fallback if keyring lib not available or backend fails: store tokens in
~/.archprime/credentials.json with chmod 0600 + a warning logged. This is
...
from archprime_cli.config import DEFAULT_HOME, Credentials, save_credentials

log = logging.getLogger("archprime_cli.auth.keyring")

KEYRING_SERVICE = "archprime-cli"
