...
### Added
- **7 nuove skill di contenuto per il conector Lovarch** (Creative Studio per i clienti): `lovarch-video`, `lovarch-storyboard`, `lovarch-carosello`, `lovarch-reel`, `lovarch-voce`, `lovarch-branding`, `lovarch-post`. Ognuna unisce il CRAFT (Seedance 2, linguaggio di camera, continuità, hook, narrativa) con le tool MCP del conector: il modello scrive la regia (zero crediti), la piattaforma genera sull'account del cliente (crediti + galleria). 14 skill totali.
...
### Fixed
- **Onboarding free rotto.** Dopo `lovarch signup` i "prossimi passi" indicavano `lovarch init` / `audit` / `run` — comandi **rimossi** nel refactor: vicolo cieco. Ora guidano alle azioni free reali: `lovarch skills install`, `lovarch cad genera`, `lovarch verifica misure`. CTA di upgrade → `app.lovarch.com/settings/credits` (Lovarch Premium).

### Added
- **I lead del CLI arrivano nel CRM.** Il `lovarch signup` (free) ora specchia il lead nel CRM ArchPrime (PrimeTeam), attribuito alla campagna **LOVARCH_CLI**, così il funnel di vendita li nutre (lato server: EF `cli-signup` → `cli-lead-ingest`).
