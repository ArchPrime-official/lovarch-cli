...
Scarica da <https://www.python.org/downloads/> e, **durante il setup, spunta
"Add python.exe to PATH"** (è il passo che quasi tutti dimenticano — senza,
`python`/`py` non funzionano dal terminale).
...
### 3. Installa il Lovarch CLI

```powershell
pipx install git+https://github.com/ArchPrime-official/lovarch-cli.git
```

> Serve **git** installato (<https://git-scm.com/download/win>). In alternativa,
> per una versione precisa:
> `pipx install git+https://github.com/ArchPrime-official/lovarch-cli.git@v0.4.3`
...
| `py` / `python` non riconosciuto | Python non è nel PATH → reinstalla Python spuntando **"Add python.exe to PATH"**. |
| `git` non riconosciuto durante `pipx install` | Installa Git for Windows (<https://git-scm.com/download/win>) e riapri il terminale. |
| Ho fatto `pip install lovarch-cli` e la versione è vecchia | Il pacchetto PyPI è fermo a una release vecchia. Disinstalla (`pip uninstall lovarch-cli`) e usa **`pipx install git+…`**. |
